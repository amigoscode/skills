...
// Self-contained: this skill bundles its own generator so it does not depend on
// any other skill being installed. Env precedence: the shell environment always
// wins, then the shared ~/amigoscode-skills/.env (one key file for every
// Amigoscode skill), then this skill's own .env (repo root) as a fallback.
// dotenv never overrides an already-set variable, and an earlier load beats a
// later one, so this gives shell > shared > skill-local.
...
if (!process.env.GEMINI_API_KEY) {
  console.error(
    "Error: GEMINI_API_KEY is not set. Export it, add it to ~/amigoscode-skills/.env, " +
      "or copy .env.example to .env in this skill. Get a key at https://aistudio.google.com/apikey"
  );
  process.exit(1);
}
